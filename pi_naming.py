...
START_WITH_LOWERCASE_REGEX = re.compile(r'[_a-z][_a-zA-Z0-9]*$')
...
class FunctionNameCheckPI(BaseASTCheck):
    """
    Function names should be start with lowercase(pi), with words separated by underscores
    as necessary to improve readability.
    Functions *not* beeing methods '__' in front and back are not allowed.
    ...
    prevailing style (e.g. threading.py), to retain backwards compatibility.
    """
    check = START_WITH_LOWERCASE_REGEX.match
...
class FunctionArgNamesCheckPI(BaseASTCheck):
    """
    The argument names of a function should be start with lowercase(pi), with words separated
    by underscores.
